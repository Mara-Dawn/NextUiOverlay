...
namespace NextUIOverlay.Gui {
	public class OverlayGui : IDisposable {
		...
		public void Initialize() {
			...
			PluginLog.Information($"WindowInfo {overlay.Size.Width}, {overlay.Size.Height}");
		...
		public void Navigate(object? sender, string newUrl) {
			...
			if (browser?.GetMainFrame().Url == newUrl) {
				PluginLog.Information($"RELOAD {newUrl}");
			...
			PluginLog.Information($"LOAD {newUrl}");
		}
		...
		public void BuildTextureWrap(Size size) {
			PluginLog.Information("0 BUILDING " + size);
			...
			PluginLog.Information("1 BUILT");
		}

		public void Dispose() {
			PluginLog.Information("Disposing overlay GUI");
			disposing = true;
			// sizeChangeSub?.Dispose();
			textureWrap?.Dispose();
			texture?.Dispose();
			PluginLog.Information("Disposed overlay GUI");
		...
		public void BrowserDispose() {
			...
			browser.Dispose();
			browser = null;
			PluginLog.Information("Browser was disposed");
		}
